---
title: Configuration
description: Configurer Bedrud selon vos besoins
order: 4
---

Bedrud uses YAML configuration files for both the main server and the embedded LiveKit media server.

**See also:** [Quick Start](/fr/docs/getting-started/quickstart) | [Installation](/fr/docs/getting-started/installation) | [Deployment Guide](/fr/docs/guides/deployment) | [Docker Guide](/fr/docs/guides/docker) | [CLI Reference](/fr/docs/getting-started/cli-reference)
...
| Champ | Signification |
|-------|---------------|
| `backend` | `disk` (défaut), `s3` ou `inline` |
| `maxBytes` | Limite dure par fichier (défaut 10 MB). Surchargeable à l'exécution via l'admin `chatUploadMaxBytes`. |
| `maxDimension` | Largeur/hauteur max en pixels (défaut 8192). Surchargeable via l'admin `chatUploadMaxDimension`. |
| `inlineMaxBytes` | Images plus petites renvoyées en data URI |
| `diskDir` | Chemin du backend disque |
| `s3.*` | Identifiants S3-compatibles et URL de base publique |

L'UI de réunion et `GET /api/auth/settings` exposent les valeurs effectives `chatUploadMaxBytes` et `chatUploadMaxDimension` pour la pré-validation côté client. L'API d'upload applique les mêmes limites (`413` trop volumineux, `400` dimensions excessives).
...
See [WebRTC Connectivity](/fr/docs/architecture/webrtc-connectivity) and [TURN Server](/fr/docs/architecture/turn-server).
...
For complete production setup, see the [Deployment Guide](/fr/docs/guides/deployment).
