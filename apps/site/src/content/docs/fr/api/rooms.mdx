...
Couper le microphone d'un participant. Seul l'administrateur de la salle peut le faire.
...
Supprimez une salle que vous avez créée. Cela supprime définitivement la salle et toutes les données associées. Exécuté en tâche de fond et retourne immédiatement.
...
| 403 | Pas le créateur de la salle (seul le créateur peut supprimer) |
| 404 | Salle introuvable |
| 409 | Suppression déjà en cours |
| 500 | Erreur serveur |

Si la salle est active, tous les participants sont déconnectés et voient le message « This room has been ended by the creator ».
...
| 403 | `"Room limit reached (max N active rooms)"` | Limite de salle atteinte |
...
| 401 | Non authentifié |
| 403 | Non autorisé (non-admin tentant une action d'admin) ou limite de salle atteinte |
| 404 | Salle non trouvée |
| 409 | Conflit (ressource en double) |
| 500 | Erreur interne du serveur |
| 507 | Quota de stockage dépassé (limite d'upload ou seuil global) |
