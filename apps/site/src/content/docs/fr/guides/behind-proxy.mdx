...
**Exigence de configuration :** si `behindProxy: true`, vous **devez** définir une liste `trustedProxies` non vide (IP/CIDR des proxies). Le serveur refuse de démarrer sans cela (pas de défaut `0.0.0.0/0`). Env : `SERVER_TRUSTED_PROXIES`.
