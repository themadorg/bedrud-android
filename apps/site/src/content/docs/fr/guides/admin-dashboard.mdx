...
description: Gérer utilisateurs, salles, paramètres et jetons d'invitation depuis l'UI web
order: 35
---
...
- **Event Types:** `room.created`, `room.ended`, `participant.joined`, `ping` (test). `recording.completed` is reserved for a planned feature. Delivery uses `X-Bedrud-Signature` / `X-Bedrud-Event` headers — see the [Webhooks Guide](/fr/docs/guides/webhooks).
...
Configure le stockage des images de chat :

- **Backend :** `disk` (système de fichiers local), `s3` (stockage compatible S3) ou `inline` (base64 intégré)
- **Max Upload Bytes** (`chatUploadMaxBytes`) : Taille maximale par fichier (défaut 10 MiB si non défini). Appliqué par l'API d'upload et exposé au client de réunion pour les pré-contrôles.
- **Max Image Dimension** (`chatUploadMaxDimension`) : Largeur ou hauteur maximale en pixels (défaut 8192 si non défini / 0). Les images décodées plus grandes sont rejetées avec HTTP 400.
- **Inline Max :** Taille maximale pour les uploads inline/base64
- **Disk Directory :** Répertoire local pour le backend `disk`

Repli YAML : `chat.uploads.maxBytes`, `chat.uploads.maxDimension` dans la [Configuration](/fr/docs/getting-started/configuration).
...
- [Admin API](/fr/docs/api/api-reference#tag/admin) — full API reference for all admin endpoints
- [Authentication](/fr/docs/backend/authentication) — access levels and role system
- [Configuration](/fr/docs/getting-started/configuration) — config.yaml settings reference
- [Quick Start](/fr/docs/getting-started/quickstart) — first admin setup walkthrough
