...
**See also:** [Quick Start](/es/docs/getting-started/quickstart) | [Installation](/es/docs/getting-started/installation) | [Deployment Guide](/es/docs/guides/deployment) | [Docker Guide](/es/docs/guides/docker) | [CLI Reference](/es/docs/getting-started/cli-reference)
...
| Campo | Significado |
|-------|-------------|
| `backend` | `disk` (predeterminado), `s3` o `inline` |
| `maxBytes` | Límite duro por archivo (predeterminado 10 MB). Se puede sobrescribir en tiempo de ejecución con el admin `chatUploadMaxBytes`. |
| `maxDimension` | Ancho/alto máximo en píxeles (predeterminado 8192). Sobrescribible con el admin `chatUploadMaxDimension`. |
| `inlineMaxBytes` | Imágenes más pequeñas devueltas como data URI |
| `diskDir` | Ruta del backend en disco |
| `s3.*` | Credenciales compatibles con S3 y URL base pública |

La UI de la reunión y `GET /api/auth/settings` exponen los valores efectivos `chatUploadMaxBytes` y `chatUploadMaxDimension` para la prevalidación del cliente. La API de subida aplica los mismos límites (`413` demasiado grande, `400` dimensiones excesivas).
...
See [WebRTC Connectivity](/es/docs/architecture/webrtc-connectivity) and [TURN Server](/es/docs/architecture/turn-server).
...
For complete production setup, see the [Deployment Guide](/es/docs/guides/deployment).
