...
**Requisito de configuración:** si `behindProxy: true`, **debe** establecer una lista `trustedProxies` no vacía (IPs/CIDR del proxy). El servidor no arranca sin ella (sin valor por defecto `0.0.0.0/0`). Env: `SERVER_TRUSTED_PROXIES`.
