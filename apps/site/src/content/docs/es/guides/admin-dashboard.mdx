...
description: Gestionar usuarios, salas, ajustes y tokens de invitación desde la UI web
order: 35
---
...
- **Recordings Toggle:** UI may expose an “allow room recordings” setting for a **planned** feature. Recording HTTP routes are not registered in production yet.
...
- **Event Types:** `room.created`, `room.ended`, `participant.joined`, `ping` (test). `recording.completed` is reserved for a planned feature. Delivery uses `X-Bedrud-Signature` / `X-Bedrud-Event` headers — see the [Webhooks Guide](/es/docs/guides/webhooks).
...
### Pestaña Chat Uploads

Configura el almacenamiento de las subidas de imágenes del chat:

- **Backend:** `disk` (sistema de archivos local), `s3` (almacenamiento compatible con S3) o `inline` (base64 embebido)
- **Max Upload Bytes** (`chatUploadMaxBytes`): Tamaño máximo por subida (predeterminado 10 MiB si no se define). Lo aplica la API de subida y se expone al cliente de la reunión para las comprobaciones previas.
- **Max Image Dimension** (`chatUploadMaxDimension`): Ancho o alto máximo en píxeles (predeterminado 8192 si no se define / 0). Las imágenes decodificadas mayores se rechazan con HTTP 400.
- **Inline Max:** Tamaño máximo para subidas inline/base64
- **Disk Directory:** Directorio local para el backend `disk`

Respaldo YAML: `chat.uploads.maxBytes`, `chat.uploads.maxDimension` en la [Configuración](/es/docs/getting-started/configuration).
...
- [Admin API](/es/docs/api/api-reference#tag/admin) — full API reference for all admin endpoints
- [Authentication](/es/docs/backend/authentication) — access levels and role system
- [Configuration](/es/docs/getting-started/configuration) — config.yaml settings reference
- [Quick Start](/es/docs/getting-started/quickstart) — first admin setup walkthrough
