...
- **TURN Port (3478 UDP / 5349 TLS):** Relays media for clients behind restrictive NATs or firewalls. See [TURN Server Guide](/es/docs/architecture/turn-server).

For firewall and port requirements, see [WebRTC Connectivity](/es/docs/architecture/webrtc-connectivity).
...
🚧 **Planned feature.** Production routes for start/stop/list/delete recordings are commented out (`// TODO oncoming feature` in `internal/server/server.go`). Implementation code (handlers, egress client, `process_recording` job, `recording.completed` webhook) exists for the upcoming release but is **not active** yet.

When enabled, the intended flow is:

1. **Start:** `StartRoomCompositeEgress` with room-scoped JWT (`RoomRecord: true` and `Room: <roomName>`).
2. **Stop:** `StopEgress`, then background `process_recording` stores the file.
3. **Complete:** Optional `recording.completed` outbound webhook.
...
- [TURN Server Guide](/es/docs/architecture/turn-server) - TURN architecture, configuration, TLS, and troubleshooting
