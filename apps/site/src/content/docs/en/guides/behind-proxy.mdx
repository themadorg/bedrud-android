...
**Config requirement:** when `behindProxy: true`, you **must** set a non-empty `trustedProxies` list (proxy IPs or CIDRs). The server refuses to start without it (no `0.0.0.0/0` default). Env: `SERVER_TRUSTED_PROXIES`.
