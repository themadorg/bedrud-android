...
- **Event Types:** `room.created`, `room.ended`, `participant.joined`, `ping` (test). `recording.completed` is reserved for a planned feature. Delivery uses `X-Bedrud-Signature` / `X-Bedrud-Event` headers — see the [Webhooks Guide](/en/docs/guides/webhooks).
...
- **Max Upload Bytes** (`chatUploadMaxBytes`): Maximum file size per upload (default 10 MiB when unset). Applied by the upload API and exposed to the meeting client for pre-checks.
- **Max Image Dimension** (`chatUploadMaxDimension`): Maximum width or height in pixels (default 8192 when unset / 0). Images that decode larger are rejected with HTTP 400.
- **Inline Max:** Maximum size for inline/base64 uploads
- **Disk Directory:** Local directory for `disk` backend uploads

YAML fallbacks: `chat.uploads.maxBytes`, `chat.uploads.maxDimension` in [Configuration](/en/docs/getting-started/configuration).
...
- [Admin API](/en/docs/api/api-reference#tag/admin) — full API reference for all admin endpoints
