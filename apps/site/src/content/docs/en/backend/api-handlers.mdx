...
Bedrud uses the **Fiber** web framework. Production routes are registered in `server/internal/server/server.go` (used by `bedrud run`). Interactive OpenAPI: [API Reference](/en/docs/api/api-reference).
...
- [Authentication](/en/docs/backend/authentication)
- [Webhooks Guide](/en/docs/guides/webhooks)
