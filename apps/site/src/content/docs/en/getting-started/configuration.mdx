...
livekit:
  ...
  apiSecret: "devsecret"        # LiveKit API secret (auto-generated if empty)
  ...
  #   External LiveKit: configure URL in LiveKit dashboard -> https://<domain>/api/livekit/webhook
...
Templates are embedded at build time and cannot be customized without recompiling Bedrud. Unknown template names fall back to a generic notification template with key-value data.
