...
ルーム作成者と昇格したモデレーターは、通話中に参加者コンテキストメニューとステージ/共有コントロールを使います。ホスト向けの動作（ミュート、キック、画面共有停止、スポットライト、ステージ）は [ミーティングコントロール](/ja/docs/using/meeting-controls) と [ステージモード](/ja/docs/using/stage-mode) を参照してください。
