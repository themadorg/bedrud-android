---
title: 管理ダッシュボード
description: Web UI でユーザー・ルーム・設定・招待トークンを管理
order: 35
---
...
- **Event Types:** `room.created`, `room.ended`, `participant.joined`, `ping` (test). `recording.completed` is reserved for a planned feature. Delivery uses `X-Bedrud-Signature` / `X-Bedrud-Event` headers — see the [Webhooks Guide](/ja/docs/guides/webhooks).
...
- **Backend:** `disk`（ローカル FS）、`s3`（S3 互換）、または `inline`（base64 埋め込み）
- **Max Upload Bytes**（`chatUploadMaxBytes`）: 1 回のアップロードの最大サイズ（未設定時は 10 MiB）。アップロード API が適用し、事前チェック用に会議クライアントへ公開します。
- **Max Image Dimension**（`chatUploadMaxDimension`）: 最大幅または高さ（ピクセル。未設定 / 0 のとき 8192）。デコード結果がこれを超える画像は HTTP 400 で拒否されます。
- **Inline Max:** inline/base64 アップロードの最大サイズ
- **Disk Directory:** `disk` バックエンドのローカルディレクトリ

YAML フォールバック: [設定](/ja/docs/getting-started/configuration) の `chat.uploads.maxBytes`、`chat.uploads.maxDimension`。
...
- [Admin API](/ja/docs/api/api-reference#tag/admin) — full API reference for all admin endpoints
- [Authentication](/ja/docs/backend/authentication) — access levels and role system
- [Configuration](/ja/docs/getting-started/configuration) — config.yaml settings reference
- [Quick Start](/ja/docs/getting-started/quickstart) — first admin setup walkthrough
