...
自分が作成したルームを削除します。ルームと関連するすべてのデータを完全に削除します。バックグラウンドジョブとして実行され、すぐに戻ります。
...
| 403 | ルーム作成者ではありません（作成者のみ削除可能） |
| 404 | ルームが見つかりません |
| 409 | 削除は既に進行中です |
| 500 | サーバーエラー |

ルームがアクティブな場合、すべての参加者が切断され、「このルームは作成者によって終了されました」というメッセージが表示されます。
...
| 403 | `"Room limit reached (max N active rooms)"` | ルーム制限に達しました |
...
| 401 | 未認証 |
| 403 | 権限なし（非管理者が管理者操作を試行）またはルーム制限に達しました |
| 404 | ルームが見つかりません |
| 409 | 競合（リソースの重複） |
| 500 | サーバー内部エラー |
| 507 | ストレージ割り当て超過（アップロード制限またはグローバルしきい値） |
