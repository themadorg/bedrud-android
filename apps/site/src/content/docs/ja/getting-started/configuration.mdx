---
title: 設定
description: ニーズに合わせて Bedrud を設定
order: 4
---

Bedrud uses YAML configuration files for both the main server and the embedded LiveKit media server.

**See also:** [Quick Start](/ja/docs/getting-started/quickstart) | [Installation](/ja/docs/getting-started/installation) | [Deployment Guide](/ja/docs/guides/deployment) | [Docker Guide](/ja/docs/guides/docker) | [CLI Reference](/ja/docs/getting-started/cli-reference)
...
| フィールド | 意味 |
|------------|------|
| `backend` | `disk`（デフォルト）、`s3`、または `inline` |
| `maxBytes` | ファイルあたりの上限（デフォルト 10 MB）。実行時に管理の `chatUploadMaxBytes` で上書き可。 |
| `maxDimension` | 最大幅/高さ（ピクセル、デフォルト 8192）。管理の `chatUploadMaxDimension` で上書き可。 |
| `inlineMaxBytes` | これ未満は data URI で返す |
| `diskDir` | disk バックエンドのパス |
| `s3.*` | S3 互換の認証情報と公開ベース URL |

会議 UI と `GET /api/auth/settings` は有効な `chatUploadMaxBytes` / `chatUploadMaxDimension` を公開し、クライアントがアップロード前に検証できるようにします。アップロード API も同じ制限を強制します（過大 `413`、寸法超過 `400`）。
...
See [WebRTC Connectivity](/ja/docs/architecture/webrtc-connectivity) and [TURN Server](/ja/docs/architecture/turn-server).
...
For complete production setup, see the [Deployment Guide](/ja/docs/guides/deployment).
