---
title: پیکربندی
description: Bedrud را مطابق نیاز خود پیکربندی کنید
order: 4
---

Bedrud uses YAML configuration files for both the main server and the embedded LiveKit media server.

**See also:** [Quick Start](/fa/docs/getting-started/quickstart) | [Installation](/fa/docs/getting-started/installation) | [Deployment Guide](/fa/docs/guides/deployment) | [Docker Guide](/fa/docs/guides/docker) | [CLI Reference](/fa/docs/getting-started/cli-reference)
...
| فیلد | معنا |
|------|------|
| `backend` | `disk` (پیش‌فرض)، `s3` یا `inline` |
| `maxBytes` | سقف سخت برای هر فایل (پیش‌فرض ۱۰ MB). در زمان اجرا با ادمین `chatUploadMaxBytes` قابل بازنویسی. |
| `maxDimension` | حداکثر عرض/ارتفاع به پیکسل (پیش‌فرض ۸۱۹۲). با ادمین `chatUploadMaxDimension` قابل بازنویسی. |
| `inlineMaxBytes` | تصاویر کوچک‌تر به‌صورت data URI |
| `diskDir` | مسیر بک‌اند دیسک |
| `s3.*` | اعتبارنامه‌های سازگار با S3 و URL پایهٔ عمومی |

UI جلسه و `GET /api/auth/settings` مقادیر مؤثر `chatUploadMaxBytes` و `chatUploadMaxDimension` را برای پیش‌اعتبارسنجی کلاینت در اختیار می‌گذارند. API آپلود همان محدودیت‌ها را اعمال می‌کند (`413` بیش‌ازحد بزرگ، `400` ابعاد بیش‌ازحد).
...
See [WebRTC Connectivity](/fa/docs/architecture/webrtc-connectivity) and [TURN Server](/fa/docs/architecture/turn-server).
...
For complete production setup, see the [Deployment Guide](/fa/docs/guides/deployment).
