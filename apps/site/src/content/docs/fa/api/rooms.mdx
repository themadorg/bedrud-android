...
اتاقی که ایجاد کرده‌اید را حذف کنید. این کار اتاق و تمام داده‌های مرتبط را به طور دائمی حذف می‌کند. به عنوان یک کار پس‌زمینه اجرا می‌شود و بلافاصله برمی‌گردد.
...
| 403 | ایجادکننده اتاق نیستید (فقط ایجادکننده می‌تواند حذف کند) |
| 404 | اتاق پیدا نشد |
| 409 | حذف در حال انجام است |
| 500 | خطای سرور |

اگر اتاق فعال باشد، همه شرکت‌کنندگان قطع می‌شوند و پیام "This room has been ended by the creator" را مشاهده می‌کنند.
...
| 403 | `"Room limit reached (max N active rooms)"` | محدودیت اتاق رسیده است |
...
| 401 | احراز هویت نشده |
| 403 | مجاز نیست (غیرمدیر در تلاش برای انجام عملکرد مدیر) یا محدودیت اتاق رسیده است |
| 404 | اتاق یافت نشد |
| 409 | تضاد (منبع تکراری) |
| 500 | خطای داخلی سرور |
| 507 | سهمیه ذخیره‌سازی بیش از حد (محدودیت آپلود یا آستانه جهانی) |
