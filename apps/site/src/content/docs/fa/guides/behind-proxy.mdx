...
**الزام پیکربندی:** وقتی `behindProxy: true` است، باید فهرست غیرخالی `trustedProxies` (IP/CIDR پراکسی) را تنظیم کنید. بدون آن سرور شروع نمی‌شود (پیش‌فرض `0.0.0.0/0` نیست). Env: `SERVER_TRUSTED_PROXIES`.
