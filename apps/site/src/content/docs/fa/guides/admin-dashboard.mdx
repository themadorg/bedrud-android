---
title: داشبورد ادمین
description: مدیریت کاربران، اتاق‌ها، تنظیمات و توکن‌های دعوت از UI وب
order: 35
---
...
- **Event Types:** `room.created`, `room.ended`, `participant.joined`, `ping` (test). `recording.completed` is reserved for a planned feature. Delivery uses `X-Bedrud-Signature` / `X-Bedrud-Event` headers — see the [Webhooks Guide](/fa/docs/guides/webhooks).
...
نحوهٔ ذخیرهٔ آپلود تصاویر چت را پیکربندی می‌کند:

- **Backend:** `disk` (فایل‌سیستم محلی)، `s3` (ذخیره‌سازی سازگار با S3) یا `inline` (base64 توکار)
- **Max Upload Bytes** (`chatUploadMaxBytes`): حداکثر اندازهٔ هر آپلود (پیش‌فرض ۱۰ MiB اگر تنظیم نشود). توسط API آپلود اعمال می‌شود و برای پیش‌بررسی در اختیار کلاینت جلسه قرار می‌گیرد.
- **Max Image Dimension** (`chatUploadMaxDimension`): حداکثر عرض یا ارتفاع به پیکسل (پیش‌فرض ۸۱۹۲ اگر تنظیم نشود / ۰). تصاویر بزرگ‌تر پس از decode با HTTP 400 رد می‌شوند.
- **Inline Max:** حداکثر اندازه برای آپلودهای inline/base64
- **Disk Directory:** پوشهٔ محلی برای بک‌اند `disk`

پشتیبان YAML: `chat.uploads.maxBytes`، `chat.uploads.maxDimension` در [پیکربندی](/fa/docs/getting-started/configuration).
...
- [Admin API](/fa/docs/api/api-reference#tag/admin) — full API reference for all admin endpoints
- [Authentication](/fa/docs/backend/authentication) — access levels and role system
- [Configuration](/fa/docs/getting-started/configuration) — config.yaml settings reference
- [Quick Start](/fa/docs/getting-started/quickstart) — first admin setup walkthrough
