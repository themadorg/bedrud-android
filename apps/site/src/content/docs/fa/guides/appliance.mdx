...
| نصب‌کننده داخلی | پیکربندی init سرویس‌ها (systemd/OpenRC/SysV)، دایرکتوری‌ها و تنظیمات |
...
پس از نصب، دو init سرویس (systemd، OpenRC یا SysV init) اجرا می‌شوند:
...
- فایل‌های init سرویس (systemd/OpenRC/SysV)
