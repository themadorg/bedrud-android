...
description: Web arayüzünden kullanıcılar, odalar, ayarlar ve davet token'larını yönetin
order: 35
---
...
- **Event Types:** `room.created`, `room.ended`, `participant.joined`, `ping` (test). `recording.completed` is reserved for a planned feature. Delivery uses `X-Bedrud-Signature` / `X-Bedrud-Event` headers — see the [Webhooks Guide](/tr/docs/guides/webhooks).
...
### Chat Uploads sekmesi

Sohbet görsel yüklemelerinin nasıl saklanacağını yapılandırır:

- **Backend:** `disk` (yerel dosya sistemi), `s3` (S3 uyumlu depolama) veya `inline` (gömülü base64)
- **Max Upload Bytes** (`chatUploadMaxBytes`): Yükleme başına en fazla dosya boyutu (ayarlanmamışsa varsayılan 10 MiB). Yükleme API'si uygular ve ön kontroller için toplantı istemcisine sunulur.
- **Max Image Dimension** (`chatUploadMaxDimension`): Piksel cinsinden en fazla genişlik veya yükseklik (ayarlanmamış / 0 ise 8192). Daha büyük çözülen görseller HTTP 400 ile reddedilir.
- **Inline Max:** Inline/base64 yüklemeler için en fazla boyut
- **Disk Directory:** `disk` arka ucu için yerel dizin

YAML yedekleri: [Yapılandırma](/tr/docs/getting-started/configuration) içinde `chat.uploads.maxBytes`, `chat.uploads.maxDimension`.
...
- [Admin API](/tr/docs/api/api-reference#tag/admin) — full API reference for all admin endpoints
- [Authentication](/tr/docs/backend/authentication) — access levels and role system
- [Configuration](/tr/docs/getting-started/configuration) — config.yaml settings reference
- [Quick Start](/tr/docs/getting-started/quickstart) — first admin setup walkthrough
