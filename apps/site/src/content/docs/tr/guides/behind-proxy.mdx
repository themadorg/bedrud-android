...
**Yapılandırma zorunluluğu:** `behindProxy: true` iken boş olmayan bir `trustedProxies` listesi (proxy IP/CIDR) **zorunludur**. Aksi halde sunucu başlamaz (`0.0.0.0/0` varsayılanı yok). Env: `SERVER_TRUSTED_PROXIES`.
