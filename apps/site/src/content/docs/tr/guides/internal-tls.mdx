...
| Sertifikanın süresi dolmuş                 | `sudo bedrud cert renew` komutunu çalıştırarak yeniden oluşturun (her 24 saatte bir 09:00'da otomatik yenileme) |
| CN uyuşmazlığı | Modern tarayıcılar CN alanını yok sayar — SAN yetkilidir. Yükleyicinin dahili sertifika oluşturmasını kullanın (önerilen) veya openssl komutuna `-addext 'subjectAltName=...'` ekleyin |
| Eksik SANs | Modern tarayıcılar CN alanını yok sayar. Yükleyicinin dahili sertifika oluşturmasını kullanın (önerilen) veya openssl komutuna `-addext 'subjectAltName=...'` ekleyin |
