...
**See also:** [Quick Start](/tr/docs/getting-started/quickstart) | [Installation](/tr/docs/getting-started/installation) | [Deployment Guide](/tr/docs/guides/deployment) | [Docker Guide](/tr/docs/guides/docker) | [CLI Reference](/tr/docs/getting-started/cli-reference)
...
| Alan | Anlam |
|------|-------|
| `backend` | `disk` (varsayılan), `s3` veya `inline` |
| `maxBytes` | Dosya başına sert limit (varsayılan 10 MB). Çalışma anında admin `chatUploadMaxBytes` ile geçersiz kılınabilir. |
| `maxDimension` | Maks. genişlik/yükseklik piksel (varsayılan 8192). Admin `chatUploadMaxDimension` ile geçersiz kılınabilir. |
| `inlineMaxBytes` | Daha küçük görseller data URI olarak döner |
| `diskDir` | Disk arka ucu yolu |
| `s3.*` | S3 uyumlu kimlik bilgileri ve genel taban URL |

Toplantı arayüzü ve `GET /api/auth/settings`, istemcinin yüklemeden önce doğrulaması için etkin `chatUploadMaxBytes` ve `chatUploadMaxDimension` değerlerini sunar. Yükleme API'si aynı limitleri uygular (`413` aşırı büyük, `400` boyut aşımı).
...
See [WebRTC Connectivity](/tr/docs/architecture/webrtc-connectivity) and [TURN Server](/tr/docs/architecture/turn-server).
...
For complete production setup, see the [Deployment Guide](/tr/docs/guides/deployment).
