---
title: 管理后台
description: 在 Web UI 中管理用户、房间、设置和邀请令牌
order: 35
---
...
- **Event Types:** `room.created`, `room.ended`, `participant.joined`, `ping` (test). `recording.completed` is reserved for a planned feature. Delivery uses `X-Bedrud-Signature` / `X-Bedrud-Event` headers — see the [Webhooks Guide](/zh/docs/guides/webhooks).
...
- **Backend：** `disk`（本地文件系统）、`s3`（S3 兼容存储）或 `inline`（base64 内嵌）
- **Max Upload Bytes**（`chatUploadMaxBytes`）：每次上传的最大文件大小（未设置时默认 10 MiB）。由上传 API 强制执行，并向会议客户端公开以供预检。
- **Max Image Dimension**（`chatUploadMaxDimension`）：最大宽或高（像素；未设置 / 0 时默认 8192）。解码后更大的图片以 HTTP 400 拒绝。
- **Inline Max：** inline/base64 上传的最大大小
- **Disk Directory：** `disk` 后端的本地目录

YAML 回退：`chat.uploads.maxBytes`、`chat.uploads.maxDimension`，见[配置](/zh/docs/getting-started/configuration)。
...
- [Admin API](/zh/docs/api/api-reference#tag/admin) — full API reference for all admin endpoints
- [Authentication](/zh/docs/backend/authentication) — access levels and role system
- [Configuration](/zh/docs/getting-started/configuration) — config.yaml settings reference
- [Quick Start](/zh/docs/getting-started/quickstart) — first admin setup walkthrough
