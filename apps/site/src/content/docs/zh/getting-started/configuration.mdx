---
title: 配置
description: 根据需要配置 Bedrud
order: 4
---

Bedrud uses YAML configuration files for both the main server and the embedded LiveKit media server.

**See also:** [Quick Start](/zh/docs/getting-started/quickstart) | [Installation](/zh/docs/getting-started/installation) | [Deployment Guide](/zh/docs/guides/deployment) | [Docker Guide](/zh/docs/guides/docker) | [CLI Reference](/zh/docs/getting-started/cli-reference)
...
| 字段 | 含义 |
|------|------|
| `backend` | `disk`（默认）、`s3` 或 `inline` |
| `maxBytes` | 单文件硬限制（默认 10 MB）。运行时可通过管理项 `chatUploadMaxBytes` 覆盖。 |
| `maxDimension` | 最大宽/高像素（默认 8192）。可通过管理项 `chatUploadMaxDimension` 覆盖。 |
| `inlineMaxBytes` | 更小的图片以 data URI 返回 |
| `diskDir` | disk 后端路径 |
| `s3.*` | S3 兼容凭证与公开基础 URL |

会议 UI 与 `GET /api/auth/settings` 会暴露有效的 `chatUploadMaxBytes` 与 `chatUploadMaxDimension`，供客户端上传前预检。上传 API 仍强制相同限制（超大 `413`，超尺寸 `400`）。
...
See [WebRTC Connectivity](/zh/docs/architecture/webrtc-connectivity) and [TURN Server](/zh/docs/architecture/turn-server).
...
For complete production setup, see the [Deployment Guide](/zh/docs/guides/deployment).
