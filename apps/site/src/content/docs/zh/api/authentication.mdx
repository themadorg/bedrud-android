...
重新发送验证邮件链接。仅适用于未验证的本地账户。通过 `verificationEmailCooldownMins` 静默执行冷却——始终返回200以防止邮箱枚举。
...
该端点始终返回202以防止邮箱枚举。重置链接在 `resetTokenTTLHours` 后过期。
...
- [/zh/docs/backend/authentication](/zh/docs/backend/authentication) - how JWT, OAuth, passkeys, email verification, and password reset work internally
