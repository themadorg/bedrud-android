...
**Требование конфигурации:** при `behindProxy: true` **нужен** непустой список `trustedProxies` (IP/CIDR прокси). Без него процесс не стартует (нет умолчания `0.0.0.0/0`). Env: `SERVER_TRUSTED_PROXIES`.
