---
title: Админ-панель
description: Управление пользователями, комнатами, настройками и токенами приглашений в веб-UI
order: 35
---
...
- **Event Types:** `room.created`, `room.ended`, `participant.joined`, `ping` (test). `recording.completed` is reserved for a planned feature. Delivery uses `X-Bedrud-Signature` / `X-Bedrud-Event` headers — see the [Webhooks Guide](/ru/docs/guides/webhooks).
...
### Вкладка Chat Uploads

Настраивает хранение загрузок изображений чата:

- **Backend:** `disk` (локальная ФС), `s3` (S3-совместимое хранилище) или `inline` (встроенный base64)
- **Max Upload Bytes** (`chatUploadMaxBytes`): Максимальный размер файла на загрузку (по умолчанию 10 MiB, если не задано). Применяется API загрузки и отдаётся клиенту встречи для предварительных проверок.
- **Max Image Dimension** (`chatUploadMaxDimension`): Максимальная ширина или высота в пикселях (по умолчанию 8192, если не задано / 0). Более крупные декодированные изображения отклоняются с HTTP 400.
- **Inline Max:** Максимальный размер для inline/base64-загрузок
- **Disk Directory:** Локальный каталог для бэкенда `disk`

YAML-запасные значения: `chat.uploads.maxBytes`, `chat.uploads.maxDimension` в [конфигурации](/ru/docs/getting-started/configuration).
...
- [Admin API](/ru/docs/api/api-reference#tag/admin) — full API reference for all admin endpoints
- [Authentication](/ru/docs/backend/authentication) — access levels and role system
- [Configuration](/ru/docs/getting-started/configuration) — config.yaml settings reference
- [Quick Start](/ru/docs/getting-started/quickstart) — first admin setup walkthrough
