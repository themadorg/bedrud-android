...
**See also:** [Quick Start](/de/docs/getting-started/quickstart) | [Installation](/de/docs/getting-started/installation) | [Deployment Guide](/de/docs/guides/deployment) | [Docker Guide](/de/docs/guides/docker) | [CLI Reference](/de/docs/getting-started/cli-reference)
...
| Feld | Bedeutung |
|------|-----------|
| `backend` | `disk` (Standard), `s3` oder `inline` |
| `maxBytes` | Hartes Limit pro Datei (Standard 10 MB). Zur Laufzeit überschreibbar über Admin `chatUploadMaxBytes`. |
| `maxDimension` | Max. Bildbreite/-höhe in Pixeln (Standard 8192). Überschreibbar über Admin `chatUploadMaxDimension`. |
| `inlineMaxBytes` | Kleinere Bilder als data-URIs |
| `diskDir` | Pfad für Disk-Backend |
| `s3.*` | S3-kompatible Zugangsdaten und öffentliche Basis-URL |

Die Meeting-UI und `GET /api/auth/settings` liefern die effektiven Werte `chatUploadMaxBytes` und `chatUploadMaxDimension`, damit Clients vor dem Upload prüfen können. Die Upload-API erzwingt dieselben Limits (`413` zu groß, `400` Dimension überschritten).
...
See [WebRTC Connectivity](/de/docs/architecture/webrtc-connectivity) and [TURN Server](/de/docs/architecture/turn-server).
...
For complete production setup, see the [Deployment Guide](/de/docs/guides/deployment).
