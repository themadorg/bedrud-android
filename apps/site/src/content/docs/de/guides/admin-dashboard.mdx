...
description: Benutzer, Räume, Einstellungen und Einladungstoken in der Web-UI verwalten
order: 35
---
...
- **Event Types:** `room.created`, `room.ended`, `participant.joined`, `ping` (test). `recording.completed` is reserved for a planned feature. Delivery uses `X-Bedrud-Signature` / `X-Bedrud-Event` headers — see the [Webhooks Guide](/de/docs/guides/webhooks).
...
Konfiguriert die Speicherung von Chat-Bild-Uploads:

- **Backend:** `disk` (lokales Dateisystem), `s3` (S3-kompatibler Speicher) oder `inline` (base64 eingebettet)
- **Max Upload Bytes** (`chatUploadMaxBytes`): Maximale Dateigröße pro Upload (Standard 10 MiB, wenn nicht gesetzt). Wird von der Upload-API angewendet und dem Meeting-Client für Vorprüfungen bereitgestellt.
- **Max Image Dimension** (`chatUploadMaxDimension`): Maximale Breite oder Höhe in Pixeln (Standard 8192, wenn nicht gesetzt / 0). Größere dekodierte Bilder werden mit HTTP 400 abgelehnt.
- **Inline Max:** Maximale Größe für Inline-/base64-Uploads
- **Disk Directory:** Lokales Verzeichnis für das `disk`-Backend

YAML-Fallbacks: `chat.uploads.maxBytes`, `chat.uploads.maxDimension` in der [Konfiguration](/de/docs/getting-started/configuration).
...
- [Admin API](/de/docs/api/api-reference#tag/admin) — full API reference for all admin endpoints
- [Authentication](/de/docs/backend/authentication) — access levels and role system
- [Configuration](/de/docs/getting-started/configuration) — config.yaml settings reference
- [Quick Start](/de/docs/getting-started/quickstart) — first admin setup walkthrough
