...
**Konfigurationsanforderung:** Bei `behindProxy: true` **muss** `trustedProxies` eine nicht leere Liste sein (Proxy-IPs/CIDRs). Ohne sie startet der Server nicht (kein Default `0.0.0.0/0`). Env: `SERVER_TRUSTED_PROXIES`.
