...
- **Event Types:** `room.created`, `room.ended`, `participant.joined`, `ping` (test). `recording.completed` is reserved for a planned feature. Delivery uses `X-Bedrud-Signature` / `X-Bedrud-Event` headers — see the [Webhooks Guide](/ar/docs/guides/webhooks).
...
يضبط كيفية تخزين رفع صور الدردشة:

- **Backend:** `disk` (نظام ملفات محلي) أو `s3` (تخزين متوافق مع S3) أو `inline` (base64 مضمّن)
- **Max Upload Bytes** (`chatUploadMaxBytes`): أقصى حجم لكل رفع (افتراضي 10 MiB عند عدم التعيين). تطبّقه واجهة الرفع ويُعرَض لعميل الاجتماع للفحوصات المسبقة.
- **Max Image Dimension** (`chatUploadMaxDimension`): أقصى عرض أو ارتفاع بالبكسل (افتراضي 8192 عند عدم التعيين / 0). تُرفض الصور الأكبر بعد فك الترميز بـ HTTP 400.
- **Inline Max:** أقصى حجم لرفعات inline/base64
- **Disk Directory:** المجلد المحلي لخلفية `disk`

بدائل YAML: `chat.uploads.maxBytes` و`chat.uploads.maxDimension` في [الإعدادات](/ar/docs/getting-started/configuration).
...
- [Admin API](/ar/docs/api/api-reference#tag/admin) — full API reference for all admin endpoints
- [Authentication](/ar/docs/backend/authentication) — access levels and role system
- [Configuration](/ar/docs/getting-started/configuration) — config.yaml settings reference
- [Quick Start](/ar/docs/getting-started/quickstart) — first admin setup walkthrough
