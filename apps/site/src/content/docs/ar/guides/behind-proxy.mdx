...
**متطلب الإعداد:** عند `behindProxy: true` **يجب** تعيين قائمة `trustedProxies` غير فارغة (عناوين/CIDR للبروكسي). يرفض الخادم التشغيل بدونها (لا افتراضي `0.0.0.0/0`). Env: `SERVER_TRUSTED_PROXIES`.
